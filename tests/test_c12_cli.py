...
stdout, stderr or files. One subprocess case proves ``python -m onyxweb`` starts.
"""
...
class Output:
    ...
    stderr: tuple[str, ...] = ()
    files: dict[str, str] = field(default_factory=dict)  # file name -> "html" or image kind
...
def test_output(
    ...
    for file_name, kind in row.files.items():
        ...
        if kind == "html":
            htmls.append(written.read_text())
        else:
...
def test_module_entry_point_runs() -> None:
    """``python -m onyxweb`` is wired to ``main``."""
    p = subprocess.run(
        [sys.executable, "-m", "onyxweb", "--help"], capture_output=True, text=True, timeout=60
    )
    assert p.returncode == 0, p.stderr
    assert p.stdout.startswith("usage: python -m onyxweb")
